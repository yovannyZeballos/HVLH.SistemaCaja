...
{
	public partial class FormListadoDocumentosComunicacionBaja : FormBase
	{
		#region . Variables .
		...
		#region . Constructor .
		public FormListadoDocumentosComunicacionBaja()
		{
			InitializeComponent();
		...
		private async Task CargarTipoDocumento()
		{
			...
			tipoDocumento = tipoDocumento.Where(x => x.Codigo == "01" || x.Codigo == "07").OrderBy(x => x.Id).ToList();
		...
		private async Task<List<ListarDocumentoDTO>> ListarDocumentos()
		{
			...
			{
				Loading.Mostrar();
				var documentos = await _servicioDocumento.ListarDocumentosParaComunicacionBaja(Fecha);
