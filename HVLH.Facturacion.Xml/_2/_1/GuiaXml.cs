...
{
	public class GuiaXml : IDocumentoXml
	{
		public IEstructuraXml Generar(IDocumentoElectronico request)
		{
			var guia = request as DocumentoElectronicoGuia;
				...
				Signature = new SignatureType
				{
					...
					DigitalSignatureAttachment = new AttachmentType
					{
						ExternalReference = new ExternalReferenceType
						{
							URI = new URIType
							{
								Value = $"#IDSign{guia.Remitente.NroDocumento}"
							}
						}
					}
				}
			...
			if (!string.IsNullOrEmpty(guia.Comprador.NroDocumento))
			{
				despatchAdvice.BuyerCustomerParty = new CustomerPartyType
				{
					Party = new PartyType
					{
						PartyIdentification = new PartyIdentificationType[]
						{
								new PartyIdentificationType
								{
									ID = new IDType
									{
										schemeID = guia.Comprador.TipoDocumento,
										schemeName = Atributos.IdentificadorDocumentoIdentidad,
										schemeAgencyName = Atributos.AgencyNameSunat,
										schemeURI = Atributos.URICatalogo06,
										Value = guia.Comprador.NroDocumento
									}
								}
						},
						PartyLegalEntity = new PartyLegalEntityType[]
						{
								new PartyLegalEntityType
								{
									RegistrationName = new RegistrationNameType
									{
										Value = guia.Comprador.NombreLegal
									}
								}
						}
					}
			...
			if (!string.IsNullOrEmpty(guia.Proveedor.NroDocumento))
			{
				despatchAdvice.SellerSupplierParty = new SupplierPartyType
				{
					Party = new PartyType
					{
						PartyIdentification = new PartyIdentificationType[]
						{
								new PartyIdentificationType
								{
									ID = new IDType
									{
										schemeID = guia.Proveedor.TipoDocumento,
										schemeName = Atributos.IdentificadorDocumentoIdentidad,
										schemeAgencyName = Atributos.AgencyNameSunat,
										schemeURI = Atributos.URICatalogo06,
										Value = guia.Proveedor.NroDocumento
									}
								}
						},
						PartyLegalEntity = new PartyLegalEntityType[]
						{
								new PartyLegalEntityType
								{
									RegistrationName = new RegistrationNameType
									{
										Value = guia.Proveedor.NombreLegal
									}
								}
						}
					}
			...
			if (!string.IsNullOrEmpty(guia.Remitente.EntidadEmisora.NumeroAutorizacion))
			{
					...
					AgentParty = new PartyType
					{
						PartyLegalEntity = new PartyLegalEntityType[]
						{
								new PartyLegalEntityType
								{
									CompanyID = new CompanyIDType
									{
										Value = guia.Remitente.EntidadEmisora.NumeroAutorizacion,
										schemeID = guia.Remitente.EntidadEmisora.CodigoEntidadAutorizadora,
										schemeName = Atributos.IdentificadorEntidadAutorizadora,
										schemeAgencyName = Atributos.AgencyNameSunat
									}
								}
			...
			if (!string.IsNullOrEmpty(guia.PuertoAeropuerto.Codigo))
			{
				despatchAdvice.Shipment.FirstArrivalPortLocation = new LocationType1
				{
					ID = new IDType
					{
						Value = guia.PuertoAeropuerto.Codigo,
						schemeAgencyName = Atributos.AgencyNameSunat,
						schemeURI = guia.PuertoAeropuerto.Tipo == "1" ? Atributos.URICatalogo63 : Atributos.URICatalogo64,
						schemeName = guia.PuertoAeropuerto.Tipo == "1" ? "Puertos" : "Aeropuertos"
					},
					LocationTypeCode = new LocationTypeCodeType
					{
						Value = guia.PuertoAeropuerto.Tipo
					},
					Name = new NameType1
					{
						Value = guia.PuertoAeropuerto.Nombre
					}
				};
			}
