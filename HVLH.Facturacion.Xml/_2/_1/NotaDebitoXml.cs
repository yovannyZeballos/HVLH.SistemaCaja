...
namespace HVLH.Facturacion.Xml._2._1
{
	public class NotaDebitoXml : IDocumentoXml
	{
		public IEstructuraXml Generar(IDocumentoElectronico request)
		{
			...
			debitNote.Id = documentoElectronico.IdDocumento;
			debitNote.IssueDate = DateTime.Parse(documentoElectronico.FechaEmision);
			debitNote.IssueTime = TimeSpan.Parse(documentoElectronico.HoraEmision);
			debitNote.DocumentCurrencyCode = documentoElectronico.Moneda;
			debitNote.LineCountNumeric = documentoElectronico.Items.Count.ToString();
			debitNote.OrderReference = documentoElectronico.OrdenCompra;
			Note note1 = new Note()
			{
				LanguajeLocaleID = "1000",
				Value = string.Format("SON: {0} {1}.", (object)NumLetra.Convertir(documentoElectronico.TotalVenta.ToString(), true), documentoElectronico.Moneda == "USD" ? (object)"DOLARES AMERICANOS" : (object)"SOLES")
			};
			debitNote.Notes.Add(note1);
			...
			debitNote.Signature = new SignatureCac()
			{
			...
			debitNote.AccountingSupplierParty = new AccountingSupplierParty()
			{
			...
			debitNote.AccountingCustomerParty = new AccountingSupplierParty()
			{
				Party = new Party()
				{
					PartyIdentification = new PartyIdentification()
			...
			debitNote.LegalMonetaryTotal.LineExtensionAmount = new PayableAmount()
			{
				CurrencyId = documentoElectronico.Moneda,
				Value = documentoElectronico.TotalValorVenta
			};
			debitNote.LegalMonetaryTotal.TaxInclusiveAmount = new PayableAmount()
			{
				CurrencyId = documentoElectronico.Moneda,
				Value = documentoElectronico.TotalPrecioVenta
			};
			debitNote.LegalMonetaryTotal.AllowanceTotalAmount = new PayableAmount()
			{
				CurrencyId = documentoElectronico.Moneda,
				Value = documentoElectronico.TotalDescuentos
			};
			debitNote.LegalMonetaryTotal.ChargeTotalAmount = new PayableAmount()
			{
				CurrencyId = documentoElectronico.Moneda,
				Value = documentoElectronico.TotalOtrosCargos
			};
			debitNote.LegalMonetaryTotal.PrepaidAmount = new PayableAmount()
			{
				CurrencyId = documentoElectronico.Moneda,
				Value = documentoElectronico.Anticipos.Sum<DocumentoAnticipo>((Func<DocumentoAnticipo, Decimal>)(x => x.Monto))
			};
			debitNote.LegalMonetaryTotal.PayableAmount = new PayableAmount()
			{
				CurrencyId = documentoElectronico.Moneda,
			...
			foreach (DetalleDocumento detalleDocumento in documentoElectronico.Items)
			{
				...
				debitNote.DebitNoteLines.Add(new InvoiceLine()
				{
					...
					TaxTotals = new List<TaxTotal>()
		  {
			...
			return (IEstructuraXml)debitNote;
		}
	}
}
