	...
	public class CreditNote : IXmlSerializable, IEstructuraXml
	{
		...
		public void WriteXml(XmlWriter writer)
		{
			...
			foreach (InvoiceLine creditNoteLine in this.CreditNoteLines)
			{
				...
				XmlWriter xmlWriter1 = writer;
				num1 = creditNoteLine.CreditedQuantity.Value;
				string str2 = num1.ToString("###0.#0", this.Formato);
				xmlWriter1.WriteValue(str2);
				writer.WriteEndElement();
				...
				XmlWriter xmlWriter2 = writer;
				num1 = creditNoteLine.LineExtensionAmount.Value;
				...
				if (creditNoteLine.AllowanceCharge.ChargeIndicator)
				{
					...
					writer.WriteValue(creditNoteLine.AllowanceCharge.Amount.Value.ToString("###0.#0", this.Formato));
					writer.WriteEndElement();
					writer.WriteEndElement();
				}
				foreach (TaxTotal taxTotal in creditNoteLine.TaxTotals)
				{
					...
					if (taxTotal.TaxSubtotal.Percent > 0M)
					{
						...
						string str4 = percent.ToString("###0.#0", this.Formato);
				...
				writer.WriteString(creditNoteLine.Price.PriceAmount.Value.ToString("###0.#0", this.Formato));
				writer.WriteEndElement();
				writer.WriteEndElement();
				writer.WriteEndElement();
			}
